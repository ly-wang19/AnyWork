...
AnyWork は正式リリース前です。カタログの構造検査、ローカルの導入ライフサイクル試験、6 ターゲットへのクロスコンパイルは通過しています。母語話者によるレビュー、モデルを実行する全評価、全 OS・アーキテクチャでのネイティブ実行、GitHub 公開リポジトリ、署名付きリリースは未達のリリース条件です。
