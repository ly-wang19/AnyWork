...
var nativeDemos = []demoRecord{
	{
		ID:   "research",
		Name: localizedText{"en": "Research to recommendation", "zh-CN": "从调研到建议", "ja": "調査から提案まで"},
		Prompt: localizedText{
		...
		Chain:   []string{"orchestrate-work", "gather-sources", "verify-claims", "compare-options", "make-decision", "write-brief", "review-deliverable"},
		Outcome: localizedText{"en": "An auditable vendor recommendation and checked decision brief.", "zh-CN": "可审计的供应商建议和经过检查的决策简报。", "ja": "監査可能なベンダー提案と検証済み意思決定ブリーフ。"},
	},
	{
		ID:   "meeting",
		Name: localizedText{"en": "Meeting to accountable execution", "zh-CN": "从会议到责任落实", "ja": "会議から責任ある実行まで"},
		Prompt: localizedText{
		...
		Chain:   []string{"orchestrate-work", "prepare-meeting", "capture-decisions-actions", "plan-work", "prioritize-work", "review-deliverable"},
		Outcome: localizedText{"en": "A decision-focused meeting and a traceable owned execution plan.", "zh-CN": "聚焦决策的会议和可追溯、责任明确的执行计划。", "ja": "意思決定に集中した会議と、追跡可能で担当が明確な実行計画。"},
	},
	{
		ID:   "automation",
		Name: localizedText{"en": "Recurring operation to safe automation", "zh-CN": "从重复运营到安全自动化", "ja": "反復運用から安全な自動化まで"},
		Prompt: localizedText{
		...
		Chain:   []string{"orchestrate-work", "diagnose-root-cause", "create-sop", "automate-routine", "review-deliverable", "run-retrospective"},
		Outcome: localizedText{"en": "A tested, recoverable automation and its operating controls.", "zh-CN": "经过测试、可恢复的自动化及其运营控制。", "ja": "検証済みで復旧可能な自動化と運用統制。"},
	},
}

func renderDemo(language, id string) (string, error) {
	var builder strings.Builder
	if id == "" {
		builder.WriteString(msg(language, "demo_title"))
		for _, demo := range nativeDemos {
			builder.WriteString(fmt.Sprintf("\n- %s: %s", demo.ID, localize(demo.Name, language)))
		}
		builder.WriteString("\n")
		builder.WriteString(msg(language, "demo_choose"))
	...
	for _, demo := range nativeDemos {
		...
		builder.WriteString(localize(demo.Name, language))
		builder.WriteString("\n\n")
		builder.WriteString(msg(language, "demo_prompt"))
		builder.WriteString("\n")
		builder.WriteString(localize(demo.Prompt, language))
		builder.WriteString("\n\n")
		builder.WriteString(msg(language, "demo_chain"))
		builder.WriteString("\n")
		builder.WriteString(strings.Join(demo.Chain, " → "))
		builder.WriteString("\n\n")
		builder.WriteString(msg(language, "demo_outcome"))
		builder.WriteString("\n")
		builder.WriteString(localize(demo.Outcome, language))
		return builder.String(), nil
	}
	return "", fmt.Errorf("unknown demo %s", id)
}
